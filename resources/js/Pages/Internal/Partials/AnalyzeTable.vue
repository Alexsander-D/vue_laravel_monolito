...
const tableHeaders = ref([
  ...
  { name: "Defeito" },
  { name: "Solução" },
  { name: "Realizar Analise" },
]);

const tableData = computed(() => {
  return props.queue.map((data) => {
    const svgIcon = `
    ...
    const button = `
      <a href="${route("analyzes.show", data.id)}">
    ...
    return {
      updated_at: data.updated_at
        ? moment.utc(data.updated_at).format("DD/MM/YY HH:mm:ss")
        : "",
      product: data.product_new,
      responsable: data.user_name,
      product_lot: data.product_lot,
      serialOrImei: data.serial_number ?? data.imei1,
...
const tableId = ref("AnalyzesTable");
</script>

<template>
  <FormSection @submitted="submitentryForm">
    <template #title> Pendentes para análise </template>

    <template #description>
      Os produtos mostrados abaixo estão em sua responsabilidade.
    </template>
