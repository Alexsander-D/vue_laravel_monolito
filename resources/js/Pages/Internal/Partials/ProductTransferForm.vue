...
const productTransferForm = () => {
  form.post(route("product_transfer.create"), {
    errorBag: "productTransferForm",
    preserveScroll: true,
    onSuccess: () => {
      form.reset();
...
const fetchProducts = async (search) => {
  try {
    const { data } = await axios.post(route("findProducts.byUser"), {
      sku: search,
...
<template>
  <FormSection @submitted="productTransferForm">
    <template #title>
    ...
    <template #form>
      ...
      <div v-show="avalaibleProduct" class="space-y-6 mt-8">
        <div>
          <label
            class="font-medium inline-block text-sm text-gray-800 mt-2.5 dark:text-neutral-200"
            for="productSelect"
