...
const filterByStatus = (status) => {
    ...
    router.get(route("Techlog.index"), { status: newStatus }, { preserveScroll: true, preserveState: true });
};
</script>

<template>
    <FormSection>
        <template #title> Laudos de atendimento </template>
        <template #description> Triagens em andamento</template>

        <template #form>

            <Datatable :thead="tableHeaders" :ajax="route('techLog.datatable')" :id="tableId" class="text-left" />
